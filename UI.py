...
class GameBoard(Toplevel):
    ...
    # def make_commands_for_buttons(self):
    #     buttons = [self.TL, self.TM, self.TR, self.ML, self.MM, self.MR, self.BL, self.BM, self.BR]
    #     for button in buttons:
    #         action_with_arg = partial(self.change_image_on_button, button)
    #         button.config(command=action_with_arg)
    ...
    def who_wins(self):
        buttons_text = [self.TL["text"], self.TM["text"], self.TR["text"], self.ML["text"], self.MM["text"], self.MR["text"], self.BL["text"], self.BM["text"], self.BR["text"]]
        ...
        if buttons_text[0] == buttons_text[4] == buttons_text[8] == "X" or buttons_text[2] == buttons_text[4] == buttons_text[6] == "X":
            print("X winner")
            self.if_X_wins()
        elif buttons_text[0] == buttons_text[4] == buttons_text[8] == "O" or buttons_text[2] == buttons_text[4] == buttons_text[6] == "O":
            print("O winner")
            self.if_O_wins()
